...
- **persistence**: Read `sdd/{change-name}/proposal` (required) and `sdd/{change-name}/spec` (optional — may not exist if running in parallel with sdd-spec). Save as `sdd/{change-name}/design`.
...
**** Do NOT create any `openspec/` directories or files. Compose the design content in memory — you will persist it in Step 4.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/design.md` Persistence `sdd/{change-name}/design`
